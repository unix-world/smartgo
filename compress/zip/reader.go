...
const zipinsecurepath bool = true // unixman, changed from use of godebug
...
// and the zipinsecurepath is set to TRUE,
...
// If any file inside the archive uses a non-local name
// (as defined by [filepath.IsLocal]) or a name containing backslashes
// and zipinsecurepath is set to TRUE,
...
func (r *Reader) init(rdr io.ReaderAt, size int64) error {
	...
	if zipinsecurepath == true { // unixman
		for _, f := range r.File {
		//	if f.Name == "" {
			if strings.TrimSpace(f.Name) == "" { // fix by unixman
