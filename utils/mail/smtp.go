...
//-- unixman
// NewDialer returns a new SMTP Dialer with most of the default options.
// The given parameters are used to connect to the SMTP server.
func NewDialer(host string, port uint16, username string, password string) (*Dialer, error) {
...
// NewCustomAuthDialer returns a new SMTP Dialer but most of the options have to be customized.
// The given parameters are used to connect to the SMTP server.
func NewCustomAuthDialer(host string, port uint16, auth smtp.Auth) (*Dialer, error) {
	host = strings.TrimSpace(host)
	if(host == "") {
		return nil, errors.New("gomail: HostName is Empty")
	}
...
func (d *Dialer) Dial() (SendCloser, error) {
	...
	if d.Auth == nil && d.Username != "" {
		if ok, auths := c.Extension("AUTH"); ok {
			if strings.Contains(auths, "CRAM-MD5") { // AUTH CRAM-MD5
				d.Auth = smtp.CRAMMD5Auth(d.Username, d.Password)
			} else if strings.Contains(auths, "XOAUTH2") && len(d.Password) >= 40 { // AUTH XOAUTH2: added by unixman ; if not explicit set, to select this expects a token as password, so shorther passwords are by default considered not tokens (shortest OAUTH2 token have github, 40 bytes ; GitLAB 64 bytes ; longest have Google 222 bytes)
				d.Auth = &loginXOauth2{
					username: d.Username,
					token:    d.Password,
					host:     d.Host,
				}
			} else if strings.Contains(auths, "LOGIN") && !strings.Contains(auths, "PLAIN") { // AUTH LOGIN
				d.Auth = &loginAuth{
					username: d.Username,
					password: d.Password,
					host:     d.Host,
