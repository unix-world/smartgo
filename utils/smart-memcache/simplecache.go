...
type CacheExpireHandlerFunc func(ce CacheEntry)
...
type CacheEntry struct {
	...
	Fn    CacheExpireHandlerFunc 	`json:"-"`    // any ; this field should not be exported to JSON, it is intended to store internal specific Go Objects
}
				...
				if(len(lc.objects) > 0) {
					for uid, cu := range lc.objects {
						if((cu.expireAtTimestamp > 0) && (cu.expireAtTimestamp <= time.Now().UTC().Unix())) {
							if(cu.CacheEntry.Fn != nil) {
								cu.CacheEntry.Fn(cu.CacheEntry) // execute destruct callback handler if not null before cache object to be unset
								if(lc.debug == true) {
									log.Println("[DEBUG] InMemCache [" + lc.name + "] :: Executing FN() on ID: `" + cu.CacheEntry.Id + "` (UID: `" + uid + "`)")
								} //end if
							} //end if
							delete(lc.objects, uid)
							if(lc.debug == true) {
								log.Println("[DEBUG] InMemCache [" + lc.name + "] :: delete UID: `" + uid + "`, (ID: `" + cu.CacheEntry.Id + "`)")
							} //end if
						} //end if
					} //end for
				} //end if
