...
import (
	...
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/cryptutil"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/datautil"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/errorutil"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/fileutil"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/httputil"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/httputil/access"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/httputil/auth"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/httputil/user"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/lockutil"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/timeutil"
	"github.com/unix-world/smartgo/db/eliasdb/api"
	"github.com/unix-world/smartgo/db/eliasdb/api/ac"
	v1 "github.com/unix-world/smartgo/db/eliasdb/api/v1"
	"github.com/unix-world/smartgo/db/eliasdb/cluster"
	"github.com/unix-world/smartgo/db/eliasdb/cluster/manager"
	"github.com/unix-world/smartgo/db/eliasdb/config"
	"github.com/unix-world/smartgo/db/eliasdb/ecal"
	"github.com/unix-world/smartgo/db/eliasdb/graph"
	"github.com/unix-world/smartgo/db/eliasdb/graph/graphstorage"
)
...
*/
//func StartServer() {
func StartServer(lockTimeoutSec uint16) { // unixman
//	StartServerWithSingleOp(nil)
	StartServerWithSingleOp(nil, lockTimeoutSec) // unixman
}
...
func StartServerWithSingleOp(singleOperation func(*graph.Manager) bool, lockTimeoutSec uint16) { // unixman
	var err error
	...
	//	gs, err = graphstorage.NewDiskGraphStorage(loc, readonly)
		gs, err = graphstorage.NewDiskGraphStorage(loc, readonly, lockTimeoutSec) // unixman
		if err != nil {
			fatal(err)
			return
		}
	...
	if config.Bool(config.EnableAccessControl) {
		...
		ac.UserDB, err = datautil.NewEnforcedUserDB(filepath.Join(basepath, config.Str(config.LocationUserDB)), UserDBPassphrase)
		...
		if err == nil {
			...
			if ok, err = fileutil.PathExists(filepath.Join(basepath, config.Str(config.LocationAccessDB))); !ok && err == nil {
				err = os.WriteFile(filepath.Join(basepath, config.Str(config.LocationAccessDB)), ac.DefaultAccessDB, smart.CHMOD_FILES) // unixman use os instead of io-util
			}
	...
	if config.Bool(config.EnableWebFolder) {
		...
		if config.Bool(config.EnableAccessControl) {
			...
			if res, _ := fileutil.PathExists(loginFile); !res {
				errorutil.AssertOk(os.WriteFile(loginFile, []byte(LoginSRC[1:]), smart.CHMOD_FILES)) // unixman use os instead of io-util
			}
		}
		...
		if config.Bool(config.EnableWebTerminal) {
			...
			if res, _ := fileutil.PathExists(termFile); !res {
				errorutil.AssertOk(os.WriteFile(termFile, []byte(TermSRC[1:]), smart.CHMOD_FILES)) // unixman use os instead of io-util
			}
		}
		...
		if config.Bool(config.EnableClusterTerminal) {
			...
			if res, _ := fileutil.PathExists(termFile); !res {
				errorutil.AssertOk(os.WriteFile(termFile, []byte(ClusterTermSRC[1:]), smart.CHMOD_FILES)) // unixman use os instead of io-util
			}
		}
	}
	...
	go hs.RunHTTPSServer(basepath+config.Str(config.LocationHTTPS), config.Str(config.HTTPSCertificate), config.Str(config.HTTPSKey), ":"+port, &wg)
	...
	if len(certs) > 0 {
		...
		os.WriteFile(fpfile, buf.Bytes(), smart.CHMOD_FILES) // unixman use os instead of io-util
	}
