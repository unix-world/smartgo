		...
		if dest, err = os.Create(*p.TargetBinary); err == nil {
			...
			if bytes, err = io.Copy(dest, source); err == nil {
				...
				if bytes, err = dest.WriteString(packmarker); err == nil {
					...
					if data, err = os.ReadFile(p.EntryFile); err == nil { // unixman use os instead of io-util
						var f io.Writer
...
func (p *CLIPacker) packFiles(w *zip.Writer, filePath string, zipPath string) error {
	var bytes int
	files, err := iox.ReadDir(filePath) // unixman use iox instead of io-util
	...
	if err == nil {
		for _, file := range files {
			if !file.IsDir() {
				...
				if data, err = os.ReadFile(diskfile); err == nil { // unixman use os instead of io-util
					var f io.Writer
		...
		for _, f := range r.File {
			if err == nil {
				if rc, err = f.Open(); err == nil {
					...
					if data, err = io.ReadAll(rc); err == nil { // unixman use io instead of io-util
