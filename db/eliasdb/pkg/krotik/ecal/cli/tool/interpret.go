...
import (
	...
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/termutil"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/ecal/config"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/ecal/engine"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/ecal/interpreter"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/ecal/parser"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/ecal/scope"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/ecal/stdlib"
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/ecal/util"
)
...
func (i *CLIInterpreter) LoadInitialFile(tid uint64) error {
	...
	if i.EntryFile != "" {
		...
		initFile, err = os.ReadFile(i.EntryFile) // unixman use os instead of io-util
...
func (i *CLIInterpreter) LoadStdlibPlugins(interactive bool) error {
	...
	if i.LoadPlugins {
		...
		if ok, _ := fileutil.PathExists(confFile); ok {
			...
			var content []byte
			if content, err = os.ReadFile(confFile); err == nil { // unixman use os instead of io-util
