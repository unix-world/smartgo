			...
			func(path string, i os.FileInfo, err error) error {
				if err == nil && !i.IsDir() {
					...
					if strings.HasSuffix(path, ext) {
						if data, err = os.ReadFile(path); err == nil { // unixman use os instead of io-util
							var ferr error

							if ast, ferr = parser.Parse(path, string(data)); ferr == nil {
								if srcFormatted, ferr = parser.PrettyPrint(ast); ferr == nil {
									os.WriteFile(path, []byte(fmt.Sprintln(srcFormatted)), i.Mode()) // unixman use os instead of io-util
								}
							}
