...
func (t *PersistedACLTable) sync(checkFile bool) error {
	...
	writeMemoryTable := func() error {
		...
		if err == nil {
			err = os.WriteFile(t.filename, res, smart.CHMOD_FILES) // unixman use os instead of io-util
		}
	...
	readMemoryTable := func() (map[string]interface{}, error) {
		...
		res, err := os.ReadFile(t.filename) // unixman use os instead of io-util
	...
	file, err := os.OpenFile(t.filename, os.O_RDONLY, smart.CHMOD_FILES)
