...
func NewMultiFileBuffer(filename string, it FilenameIterator, cond RolloverCondition) (*MultiFileBuffer, error) {
	...
	if mfb.fp == nil {
		...
		mfb.lock.Lock()
		mfb.fp, err = os.OpenFile(mfb.basename, os.O_APPEND|os.O_RDWR, smart.CHMOD_FILES)
		mfb.lock.Unlock()
...
func (mfb *MultiFileBuffer) rollover() error {
	...
	if err == nil {
		...
		if err == nil {
			...
			mfb.fp, err = os.OpenFile(mfb.basename, os.O_CREATE|os.O_TRUNC|os.O_RDWR, smart.CHMOD_FILES)
		}
	}
...
func (it *dailyDateIterator) Basename(filename string) string {
	...
	if it.maxDays > 0 {
		...
		if files, err := iox.ReadDir(dir); err == nil { // unixman use iox instead of io-util
