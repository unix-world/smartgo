...
func LoadConfig(filename string, defaultConfig map[string]interface{}) (map[string]interface{}, error) {
	...
	if ok, err = PathExists(filename); err != nil {
		...
		mdata, err = os.ReadFile(filename) // unixman use os instead of io-util
		if err == nil {
		...
		if err == nil {
			...
			err = os.WriteFile(filename, mdata, smart.CHMOD_FILES) // unixman use os instead of io-util
		}
	}
...
func (wc *WatchedConfig) sync(checkFile bool) error {
	...
	writeMemoryTable := func() error {
		...
		if err == nil {
			err = os.WriteFile(wc.filename, res, smart.CHMOD_FILES) // unixman use os instead of io-util
		}
	...
	readMemoryTable := func() (map[string]interface{}, error) {
		...
		res, err := os.ReadFile(wc.filename) // unixman use os instead of io-util
	...
	file, err := os.OpenFile(wc.filename, os.O_RDONLY, smart.CHMOD_FILES)
