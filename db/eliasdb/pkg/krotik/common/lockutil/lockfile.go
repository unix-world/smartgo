...
//	return &LockFile{filename, time.Now().UnixNano(), interval, nil, false, &sync.Mutex{}}
	return &LockFile{filename, time.Now().UnixNano(), interval, nil, false, &sync.RWMutex{}} // unixman
}
...
//	file, err := os.OpenFile(lf.filename, os.O_CREATE|os.O_TRUNC|os.O_RDWR, smart.CHMOD_FILES)
	file, err := os.OpenFile(lf.filename, os.O_EXCL|os.O_CREATE|os.O_TRUNC|os.O_RDWR, smart.CHMOD_FILES) // fix by unixman
	if err != nil {
		return err
	}
	defer file.Close()
...
//	defer lf.mutex.Unlock()
	...
	file, err := os.OpenFile(lf.filename, os.O_EXCL|os.O_RDONLY, smart.CHMOD_FILES)
	if err != nil {
