...
import (
	...
	"github.com/unix-world/smartgo/db/eliasdb/storage/file"
	"github.com/unix-world/smartgo/db/eliasdb/storage/paging"
	"github.com/unix-world/smartgo/db/eliasdb/storage/slotting"
	"github.com/unix-world/smartgo/db/eliasdb/storage/util"
)
...
func NewDiskStorageManager(filename string, readonly bool, onlyAppend bool, transDisabled bool, lockfileDisabled bool, lockTimeoutSec uint16) *DiskStorageManager { // unixman
//	return &DiskStorageManager{NewByteDiskStorageManager(filename, readonly, onlyAppend, transDisabled, lockfileDisabled)}
	return &DiskStorageManager{NewByteDiskStorageManager(filename, readonly, onlyAppend, transDisabled, lockfileDisabled, lockTimeoutSec)} // unixman
}
...
*/
//func NewByteDiskStorageManager(filename string, readonly bool, onlyAppend bool, transDisabled bool, lockfileDisabled bool) *ByteDiskStorageManager {
func NewByteDiskStorageManager(filename string, readonly bool, onlyAppend bool, transDisabled bool, lockfileDisabled bool, lockTimeoutSec uint16) *ByteDiskStorageManager { // unixman
	...
	if !lockfileDisabled {
	//	lf = lockutil.NewLockFile(fmt.Sprintf("%v.%v", filename, FileSiffixLockfile), time.Duration(50) * time.Millisecond)
		lf = lockutil.NewLockFile(fmt.Sprintf("%v.%v", filename, FileSiffixLockfile), time.Duration(25) * time.Millisecond) // unixman
	}

	bdsm := &ByteDiskStorageManager{filename, readonly, onlyAppend, transDisabled, &sync.Mutex{}, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, lf}

//	err := initByteDiskStorageManager(bdsm)
	err := initByteDiskStorageManager(bdsm, lockTimeoutSec) // uixman
	if err != nil {
...
func initByteDiskStorageManager(bdsm *ByteDiskStorageManager, timeoutSec uint16) error { // unixman
	...
	if bdsm.lockfile != nil {
		...
		if err := bdsm.lockfile.Start(); err != nil {
			panic(fmt.Sprintf("Could not take ownership of lockfile %v: %v", bdsm.filename, err))
			return err // fix by unixman
		}
		*/
		//--
		if(timeoutSec <= 0) {
		...
		var loopCycles int = 10 * int(timeoutSec)
		//--
		var lockErr error = nil
		for i:=0; i<loopCycles; i++ { // try to achieve a lock up to 10 seconds
			lockErr = bdsm.lockfile.Start()
			if(lockErr == nil) {
				break
			} //end if
		...
		if(lockErr != nil) {
			panic(fmt.Sprintf("Could not take ownership of lockfile %v: %v", bdsm.filename, lockErr))
			return lockErr
		}
		//-- #end fix
	}
