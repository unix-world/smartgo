...
import (
	...
	"github.com/unix-world/smartgo/db/eliasdb/console"
	"github.com/unix-world/smartgo/db/eliasdb/graph"
	"github.com/unix-world/smartgo/db/eliasdb/server"
)

//-- unixman
const (
	LOCKFILE_TIMEOUT_SECONDS uint16 = 10
)
//-- #
...
func main() {
	...
	if len(flag.Args()) > 0 {
		...
		//	server.StartServerWithSingleOp(handleServerCommandLine)
			server.StartServerWithSingleOp(handleServerCommandLine, LOCKFILE_TIMEOUT_SECONDS) // unixman
		} else if arg == "console" {
			config.LoadConfigFile(config.DefaultConfigFile)
			RunCliConsole()
		} else {
	...
	if err == nil {
		...
		con := console.NewConsole(fmt.Sprintf("https://%s:%s", *host, *port), os.Stdout,
			...
			func(args []string, exportBuf *bytes.Buffer) error {
				...
				return os.WriteFile(filename, exportBuf.Bytes(), smart.CHMOD_FILES) // unixman use os instead of io-util
			})
