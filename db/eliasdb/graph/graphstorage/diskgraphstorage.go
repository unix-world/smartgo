...
// contains fixes + modifications by unixman
...
const FilenameNameDB = "name.pm" // unixman
...
type DiskGraphStorage struct {
	...
	lockTimeoutSec  uint16                        // unixman
}

/*
NewDiskGraphStorage creates a new DiskGraphStorage instance.
*/
func NewDiskGraphStorage(name string, readonly bool, lockTimeoutSec uint16) (Storage, error) {

//	dgs := &DiskGraphStorage{name, readonly, nil, make(map[string]storage.Manager)}
	dgs := &DiskGraphStorage{name, readonly, nil, make(map[string]storage.Manager), lockTimeoutSec} // unixman
...
func (dgs *DiskGraphStorage) StorageManager(smname string, create bool) storage.Manager {
	...
	//	dsm := storage.NewDiskStorageManager(dgs.name+"/"+smname, dgs.readonly, false, false, false)
		dsm := storage.NewDiskStorageManager(dgs.name+"/"+smname, dgs.readonly, false, false, false, dgs.lockTimeoutSec)
