...
// contains modifications by unixman:
// 	* modified storage files name from `part+kind` to `part+"-"+kind`
// 	* partition is enforced between 3..25 characters to sync with SF (safe) username
// 	* stay safe with max filename 255 chars, limit node kind to max 64 characters, to sync with edge role1+role2 (which also are limited to max 64 characters, and have to stay sync with nodes ...)
...
import (
	...
	"github.com/unix-world/smartgo/db/eliasdb/pkg/krotik/common/stringutil"
	"github.com/unix-world/smartgo/db/eliasdb/graph/data"
	"github.com/unix-world/smartgo/db/eliasdb/graph/util"
	"github.com/unix-world/smartgo/db/eliasdb/hash"
	"github.com/unix-world/smartgo/db/eliasdb/storage"
)
...
//	if !stringutil.IsAlphaNumeric(part) {
	if((len(part) < 3) || (len(part) > 25) || !stringutil.IsAlphaNumeric(part)) { // unixman ; sync with the SF: SAFE-USERNAME min/max length
		...
		//	Detail: fmt.Sprintf("Partition name %v is not alphanumeric - can only contain [a-zA-Z0-9_]", part),
			Detail: fmt.Sprintf("Partition name %v is not alphanumeric - can only contain [a-zA-Z0-9_] ; 3..25 characters", part),
		}
	}
...
func (gm *Manager) checkItemGeneral(node data.Node, name string) error {
	...
	if(len(node.Key()) > 255) { // unixman
		return &util.GraphError{Type: util.ErrInvalidData, Detail: name + " key is too long, can have max 255 characters"}
	}
...
//	if !stringutil.IsAlphaNumeric(node.Kind()) {
	if((len(node.Kind()) > 64) || !stringutil.IsAlphaNumeric(node.Kind())) { // unixman: allow max 64 characters in node kind
		return &util.GraphError{
			Type:   util.ErrInvalidData,
		//	Detail: fmt.Sprintf("%v kind %v is not alphanumeric - can only contain [a-zA-Z0-9_]", name, node.Kind()),
			Detail: fmt.Sprintf("%v kind %v is not alphanumeric - can only contain [a-zA-Z0-9_] ; no more than 64 characters", name, node.Kind()),
		}
	}
...
func (gm *Manager) checkEdge(edge data.Edge) error {
	...
	} else if(len(edge.End1Key()) > 255) { // unixman
		return &util.GraphError{Type: util.ErrInvalidData, Detail: "Edge is key is too long, can have max 255 characters"}
	}

	if edge.End1Kind() == "" {
		return &util.GraphError{Type: util.ErrInvalidData, Detail: "Edge is missing a kind value for end1"}
	} else if((len(edge.End1Kind()) > 64) || !stringutil.IsAlphaNumeric(edge.End1Kind())) { // unixman: limit this to max 64 characters
		return &util.GraphError{
			Type:   util.ErrInvalidData,
			Detail: fmt.Sprintf("Edge kind %v is not alphanumeric - can only contain [a-zA-Z0-9_] ; max 64 characters", edge.End1Kind()),
		}
	}
...
//	} else if !stringutil.IsAlphaNumeric(edge.End1Role()) {
	} else if((len(edge.End1Role()) > 64) || !stringutil.IsAlphaNumeric(edge.End1Role())) { // unixman: limit this to max 64 characters
		return &util.GraphError{
			Type:   util.ErrInvalidData,
		//	Detail: fmt.Sprintf("Edge role %v is not alphanumeric - can only contain [a-zA-Z0-9_]", edge.End1Role()),
			Detail: fmt.Sprintf("Edge role %v is not alphanumeric - can only contain [a-zA-Z0-9_] ; max 64 characters", edge.End1Role()),
		}
	}

	if _, ok := edge.Attr(data.EdgeEnd1Cascading).(bool); !ok {
	//	return &util.GraphError{Type: util.ErrInvalidData, Detail: "Edge is missing a cascading value for end1"} // unixman, fix import with broken edge cascading attribute
	}

	if edge.End2Key() == "" {
		return &util.GraphError{Type: util.ErrInvalidData, Detail: "Edge is missing a key value for end2"}
	} else if(len(edge.End2Key()) > 255) { // unixman
		return &util.GraphError{Type: util.ErrInvalidData, Detail: "Edge is key is too long, can have max 255 characters"}
	}

	if edge.End2Kind() == "" {
		return &util.GraphError{Type: util.ErrInvalidData, Detail: "Edge is missing a kind value for end2"}
	} else if((len(edge.End2Kind()) > 64) || !stringutil.IsAlphaNumeric(edge.End2Kind())) {
		return &util.GraphError{
			Type:   util.ErrInvalidData,
			Detail: fmt.Sprintf("Edge kind %v is not alphanumeric - can only contain [a-zA-Z0-9_] ; max 64 characters", edge.End2Kind()),
		}
	}
...
//	} else if !stringutil.IsAlphaNumeric(edge.End2Role()) {
	} else if((len(edge.End2Role()) > 64) || !stringutil.IsAlphaNumeric(edge.End2Role())) {
		return &util.GraphError{
			Type:   util.ErrInvalidData,
		//	Detail: fmt.Sprintf("Edge role %v is not alphanumeric - can only contain [a-zA-Z0-9_]", edge.End2Role()),
			Detail: fmt.Sprintf("Edge role %v is not alphanumeric - can only contain [a-zA-Z0-9_] ; max 64 characters", edge.End2Role()),
		}
	}

	if _, ok := edge.Attr(data.EdgeEnd2Cascading).(bool); !ok {
	//	return &util.GraphError{Type: util.ErrInvalidData, Detail: "Edge is missing a cascading value for end2"} // unixman, fix import with broken edge cascading attribute
	}
...
*/
func (gm *Manager) getNodeStorageHTree(part string, kind string, create bool) (*hash.HTree, *hash.HTree, error) {
...
//	if !stringutil.IsAlphaNumeric(kind) {
	if((len(kind) > 64) || !stringutil.IsAlphaNumeric(kind)) { // unixman: allow max 64 characters in node kind
		return nil, nil, &util.GraphError{
			Type:   util.ErrInvalidData,
		//	Detail: fmt.Sprintf("Node kind %v is not alphanumeric - can only contain [a-zA-Z0-9_]", kind),
			Detail: fmt.Sprintf("Node kind %v is not alphanumeric - can only contain [a-zA-Z0-9_] ; no more than 64 characters", kind), // unixman
		}
	}
...
//	if !stringutil.IsAlphaNumeric(kind) {
	if((len(kind) > 64) || !stringutil.IsAlphaNumeric(kind)) { // unixman: allow max 64 characters in node kind
		...
		//	Detail: fmt.Sprintf("Edge kind %v is not alphanumeric - can only contain [a-zA-Z0-9_]", kind),
			Detail: fmt.Sprintf("Edge kind %v is not alphanumeric - can only contain [a-zA-Z0-9_], no more than 64 characters", kind),
		}
	}
...
//	if !stringutil.IsAlphaNumeric(kind) {
	if((len(kind) > 64) || !stringutil.IsAlphaNumeric(kind)) { // unixman: allow max 64 characters in node kind
		...
		//	Detail: fmt.Sprintf("%v kind %v is not alphanumeric - can only contain [a-zA-Z0-9_]", name, kind),
			Detail: fmt.Sprintf("%v kind %v is not alphanumeric - can only contain [a-zA-Z0-9_] ; no more than 64 characters", name, kind),
		}
	}
