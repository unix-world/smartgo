...
`)
	...
	var hasEnding = false // fix by unixman
	for ik, it := range iters {
		...
		for i := 0; it.HasNext(); i++ {
			...
			if(node != nil) { // fix by unixman

				// Fetch all connected relationships and store their key and kind
			//	_, edges, err := gm.TraverseMulti(part, key, kinds[ik], ":::", false)
				_, edges, err := gm.TraverseMulti(part, node.Key(), kinds[ik], ":::", false) // fix by unixman to avoid error below
				//-- fix by unixman
			//	if err != nil {
			//		return err
			//	}
	...
	if(!hasEnding) { // fix by unixman, make json be valid, cannot end with comma
		fmt.Fprint(out, "    {}\n")
	}
...
`)
	...
	hasEnding = false // fix by unixman
	ie := 0
	for key, kind := range edgeKeys {
		...
		if(edge != nil) { // bug fix by unixman
			// Write out JSON object
			fmt.Fprint(out, "    {\n")
			writeData(edge.Data())
