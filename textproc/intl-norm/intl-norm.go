...
// Go Lang International Text Unicode Normalization: NFD Deaccent
// (c) 2021-present unix-world.org # inspired from: stackoverflow.com/questions # remove-accents-diacritics-in-a-string-in-javascript
// v.20250208.2358
// License: BSD
...
func RemoveDiacritics(str string) (string, error) {
	//--
	// example: "Têstôöłæ" -> "Testoolae"
	//--
	errInit := diacriticsInitMap()
	if(errInit != nil) {
		return "", errInit
	} //end if
	//--
	re, errRx := regexp.Compile("[^\u0000-\u007E]")
	if(errRx != nil) {
		return "", errRx
	} //end if
	if(re == nil) {
		return "", errors.New("Regex is Null")
	} //end if
	//--
	ns := re.ReplaceAllStringFunc(str, func(a string) string {
...
func diacriticsInitMap() error {
	//--
	if(len(diacriticsMap) > 0) {
		return nil // ok
	} //end if
	//--
	...
	//--
	if(len(diacriticsMap) <= 0) {
		return errors.New("Failed to Init Diacritics Map") // fail
	} //end if
	//--
	return nil // ok
	//--
