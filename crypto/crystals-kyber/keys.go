...
//func (k *Kyber) UnpackPK(packedPK []byte) *PublicKey { // modified by unixman
func (k *Kyber) UnpackPK(packedPK []byte) (error, *PublicKey) {
	if len(packedPK) != k.params.SIZEPK {
		return errors.New("Cannot unpack this public key"), nil
	}
	return nil, &PublicKey{Rho: packedPK[k.params.K*polysize:], T: unpack(packedPK[:], k.params.K)}
}
...
//func (k *Kyber) UnpackPKESK(psk []byte) *PKEPrivateKey { // modified by unixman
func (k *Kyber) UnpackPKESK(psk []byte) (error, *PKEPrivateKey) {
	if len(psk) != k.params.SIZEPKESK {
		return errors.New("Cannot unpack this private PKE key"), nil
	}
	return nil, &PKEPrivateKey{S: unpack(psk[:], k.params.K)}
}
...
//func (k *Kyber) UnpackSK(psk []byte) *PrivateKey { // modified by unixman
func (k *Kyber) UnpackSK(psk []byte) (error, *PrivateKey) {
	if len(psk) != k.params.SIZESK {
		return errors.New("Cannot unpack this private key"), nil
	}
	SIZEPKESK := k.params.SIZEPKESK
	SIZEPK := k.params.SIZEPK
	return nil, &PrivateKey{Z: psk[SIZEPKESK+SIZEPK : SIZEPKESK+SIZEPK+32], SkP: psk[:SIZEPKESK], Pk: psk[SIZEPKESK : SIZEPKESK+SIZEPK]}
}
