// Package edx448 implements Edx448 signature scheme, using SHA3-512 instead of SHAKE256.
...
// modified by unixman r.20260216: use sha3-512 / 64 / 114, instead of sha3-shake256
...
import (
	...
	"github.com/unix-world/smartgo/crypto/eddsa/internal/ecc/goldilocks"
...
func (priv PrivateKey) Sign(rand io.Reader, message []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	...
	default:
		return nil, errors.New("edx448: bad hash algorithm")
	}
}
...
func newKeyFromSeed(privateKey, seed []byte) {
	if l := len(seed); l != SeedSize {
		panic("edx448: bad seed length: " + strconv.Itoa(l))
	}

	var h [hashSize]byte
//	H := sha3.NewShake256()
	H := sha3.New512X114() // unixman
	_, _ = H.Write(seed)
	_, _ = H.Read(h[:])
	s := &goldilocks.Scalar{}
...
func signAll(signature []byte, privateKey PrivateKey, message, ctx []byte, preHash bool) {
	if len(ctx) > ContextMaxSize {
		panic(fmt.Errorf("edx448: bad context length: %v", len(ctx)))
	}

//	H := sha3.NewShake256()
	H := sha3.New512S() // unixman: for message hash only
	var PHM []byte
	if preHash {
		var h [64]byte
		_, _ = H.Write(message)
	...
	H = sha3.New512X114() // unixman: for the rest
