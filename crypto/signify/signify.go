...
// compliant with OpenBSD's signify utility standards
// License: BSD
...
func ReadData(data []byte) (comment string, content []byte, err error) { // added by unixman
	res := bytes.NewBuffer(data)
	return ReadFile(res)
}


func WriteData(comment string, content []byte) ([]byte, error) { // added by unixman
	res := bytes.Buffer{}
	err := WriteFile(&res, comment, content)
	if(err != nil) {
		return nil, err
	}
