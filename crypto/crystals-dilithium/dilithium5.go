...
//func (d *Dilithium) KeyGen(seed []byte) ([]byte, []byte) { // change by unixman
func (d *Dilithium) KeyGen(seed []byte) (error, []byte, []byte) { // return: err, publicKey, privateKey
...
//	}
	if(len(seed) != SEEDBYTES) {
		if(seed == nil) {
			...
			return errors.New("Seed must be exactly 32 bytes"), nil, nil
		} //end if else
	} //end if else
	//-- #end fix
	...
	return nil, d.PackPK(PublicKey{T1: t1, Rho: rho}), d.PackSK(PrivateKey{Rho: rho, Key: key, Tr: tr, S1: s1, S2: s2, T0: t0})
}
...
//func (d *Dilithium) Sign(packedSK, msg []byte) []byte { // modified by unixman
func (d *Dilithium) Sign(packedSK, msg []byte) (error, []byte) {
	if len(packedSK) != d.SIZESK() {
		return errors.New("Cannot sign with this key, size does not match"), nil
	}
	K := d.params.K
	L := d.params.L
	BETA := d.params.BETA
...
rej:
//	if nonce > 500 { // modified by unixman
	if(nonce > 512) {
		return errors.New("Sign ran out of trials, max=512"), nil // Failing after 512 trials happens with probability close to 2^(-128).
	}
	...
	if bytes.Equal(zero[:], hc[:]) {
		return errors.New("Internal Error: Bytes equal zero"), nil
	}
	state.Reset()
