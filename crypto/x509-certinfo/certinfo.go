...
func printName(names []pkix.AttributeTypeAndValue, buf *bytes.Buffer) []string {
	...
	for _, name := range names {
		...
		if len(oid) == 4 && oid[0] == 2 && oid[1] == 5 && oid[2] == 4 {
			switch oid[3] {
				...
				case 4:
					values = append(values, fmt.Sprintf("SN=%s", name.Value)) // unixman
				case 5:
					values = append(values, fmt.Sprintf("SerialNumber=%s", name.Value)) // unixman
				case 6:
					values = append(values, fmt.Sprintf("C=%s", name.Value))
				case 7:
					values = append(values, fmt.Sprintf("L=%s", name.Value)) // unixman
				case 8:
					values = append(values, fmt.Sprintf("ST=%s", name.Value))
				case 9:
					values = append(values, fmt.Sprintf("Street=%s", name.Value)) // unixman
				case 10:
					values = append(values, fmt.Sprintf("O=%s", name.Value))
				case 11:
					values = append(values, fmt.Sprintf("OU=%s", name.Value))
				case 17:
					values = append(values, fmt.Sprintf("PostalCode=%s", name.Value)) // unixman
				case 41:
					values = append(values, fmt.Sprintf("Name=%s", name.Value)) // unixman
				case 42:
					values = append(values, fmt.Sprintf("GN=%s", name.Value)) // unixman
				case 97:
					values = append(values, fmt.Sprintf("OrganizationIdentifier=%s", name.Value)) // unixman
				default:
					values = append(values, fmt.Sprintf("OID[%s]=%s", name.Type.String(), name.Value)) // fix by unixman: show also value
			}
	...
	if len(values) > 0 {
		...
		//	buf.WriteString("," + values[i])
			buf.WriteString("|" + values[i]) // unixman
		}
		buf.WriteString("\n")
	}
	return values
	...
	switch pkAlgo {
		case x509.RSA:
			...
			if rsaKey, ok := pk.(*rsa.PublicKey); ok {
				...
				// Some implementations (notably OpenSSL) prepend 0x00 to the modulus if its most-significant bit is set.
				// There is no need to do that here because the modulus is always unsigned and the extra byte can be confusing given the bit length.
		...
		case x509.ECDSA:
			...
			if ecdsaKey, ok := pk.(*ecdsa.PublicKey); ok {
				...
				return errors.New("certinfo: Expected ecdsa.PublicKey for type x509.ECDSA")
			}
		case x509.Ed25519: // unixman
			buf.WriteString(fmt.Sprintf("EdDSA\n"))
			if eddsaKey, ok := pk.(ed25519.PublicKey); ok {
				buf.WriteString(fmt.Sprintf("%16sEd25519 Public-Key: (%d bit)\n", "", len(eddsaKey)))
				dsaKeyPrinter("pub", big.NewInt(0).SetBytes(eddsaKey), buf)
				buf.WriteString(fmt.Sprintf("%16sCurve: %s\n", "", "Edwards25519"))
			} else {
				return errors.New("certinfo: Expected ecdsa.PublicKey for type x509.Ed25519")
			}

		default:
			return errors.New("certinfo: Unknown public key type: " + pkAlgo.String())
	}
	return nil
}
...
//-- unixman
func CertificatePEMToX509(pemCertificate string) (error, *x509.Certificate) {
	//--
	if(pemCertificate == "") {
		return errors.New("The PEM Certificate is Empty"), nil
	} //end if
	//--
	block, _ := pem.Decode([]byte(pemCertificate))
	if(block == nil) {
		return errors.New("Failed to decode PEM Certificate"), nil
	} //end if
	if(block.Type != "CERTIFICATE") {
		return errors.New("Invalid PEM Certificate Type: `" + block.Type + "`"), nil
	} //end if
	//--
	cert, err := x509.ParseCertificate(block.Bytes)
	if(err != nil) {
		return errors.New("Failed to parse Certificate: " + err.Error()), nil
	} //end if
	if(cert == nil) {
		return errors.New("Failed to parse Certificate, is Null"), nil
	} //end if
	//--
	return nil, cert
	//--
			...
			if len(ext.Id) == 4 && ext.Id[0] == 2 && ext.Id[1] == 5 && ext.Id[2] == 29 {
				...
				case 15:
					...
					if cert.KeyUsage&x509.KeyUsageContentCommitment > 0 {
						usages = append(usages, "Content Commitment (Non Repudiation)")
					}
