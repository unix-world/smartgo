...
// Encode takes a slice of bytes and encodes it to base62 string.
...
// If provided string contains characters illegal for base62 the returned error will be <notnil>
func Decode(input string) (output []byte, err error) {
	...
	for i := len(input) - 1; i >= 0; i-- {
		...
		if tmp == 255 {
			err = fmt.Errorf("invalid Base62 input string at character \"%s\", position %d", string(input[i]), i)
			return
		}
