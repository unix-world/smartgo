...
const (
	uxmDateTimeFormat string = "2006-01-02 15:04:05 -0700"

	uxmTextAllBodyPartsIncludded string = "ALL:[Attachments,Embedds:Text/Html/*,*]"
)
...
type Verification struct {
	CustomVerifier bool 		`json:"customVerifier"`
	...
	Algo string 				`json:"algo"` // unixman
	HashAlgo string 			`json:"hashAlgo"` // unixman

	CanonizeMethodHeader string `json:"canonizeMethodHeader"` // unixman
	CanonizeMethodBody string 	`json:"canonizeMethodBody"` // unixman

	// The list of signed header fields.
	HeaderKeys []string 		`json:"includeHeaderKeys"`

	IncludeBodyParts string 	`json:"includeBodyParts"` // unixman ; need to export this just to be visible in JSON

	// The time that this signature was created. If unknown, it's set to zero.
	Time time.Time 				`json:"-"`
	CreateTime string 			`json:"createTime"` // unixman omitempty
	// The expiration time. If the signature doesn't expire, it's set to zero.
	Expiration time.Time 		`json:"-"`
	ExpireTime string 			`json:"expireTime,omitempty"` // unixman omitempty

	VerifyTime string 			`json:"verifyTime"` // unixman ; need to export this just to be visible in JSON

	// Err is nil if the signature is valid.
	Err error 					`json:"verifyError"`
}
...
func verify(h header, r io.Reader, sigField, sigValue string, options *VerifyOptions) (*Verification, error) {
	...
	for _, tag := range requiredTags {
		if _, ok := params[tag]; !ok {
			return verif, permFailError("signature missing required tag: " + tag)
		}
	}
	...
	verif.VerifyTime = time.Now().Format(uxmDateTimeFormat) // unixman
	if timeStr, ok := params["t"]; ok {
		...
		verif.Time = t
		verif.CreateTime = t.Format(uxmDateTimeFormat) // unixman
	}
	if expiresStr, ok := params["x"]; ok {
		...
		verif.Expiration = t
		verif.ExpireTime = t.Format(uxmDateTimeFormat) // unixman
		if now().After(t) {
	...
	for _, method := range methods {
		if query, ok := queryMethods[QueryMethod(method)]; ok {
			if options != nil {
				if(options.LookupTXT != nil) {
					verif.CustomVerifier = true
				}
	...
	switch hashAlgo { // {{{SYNC-SUPPORTED-DKIM-ALGOS}}}
		//--
		case "sha1": // RFC 8301 section 3.1: rsa-sha1 MUST NOT be used for signing or verifying.
		...
		default:
			return verif, permFailError("unsupported hash algorithm: " + hashAlgo)
	}
	verif.HashAlgo = hashAlgo

	// Check key algo
	if res.KeyAlgo != keyAlgo {
		return verif, permFailError("inappropriate key algorithm, expects: `" + keyAlgo + "` but have: `" + res.KeyAlgo + "`")
	}
	...
	var theCanM string = params["c"]

	//-- unixman
	arrCan := strings.Split(strings.ToLower(strings.TrimSpace(theCanM)), "/")
	if(len(arrCan) == 2) {
		verif.CanonizeMethodHeader = arrCan[0]
		verif.CanonizeMethodBody = arrCan[1]
	}
	//-- #

	headerCan, bodyCan := parseCanonicalization(theCanM)
	...
	verif.IncludeBodyParts = uxmTextAllBodyPartsIncludded // unixman (need to export this just to be visible in JSON)
	...
	if err := res.Verifier.Verify(hash, hashed, sig); err != nil {
		return verif, failError("signature is not matching: " + err.Error())
	}
