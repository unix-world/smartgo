...
const (
	...
	TxtRecordPrefix string = "v=spf1"
)

var (
	ErrEmptyRecord      = errors.New("SPF Record is empty.") // unixman
...
type SPF struct { // unixman: added JSON hints
	Raw        string 		`json:"-"`
	Domain     string 		`json:"-"`
...
func getSPFRecord(domain string) (string, error) {
	...
	for _, record := range records {
		if strings.HasPrefix(record, TxtRecordPrefix) {
			spfText = record
			break
		}
	}
...
func NewSPFByRecord(domain string, record string) (SPF, error) { // by unixman, if record is empty will not lookup for it using getSPFRecord() but instead will return an error
	return newSPFWithMandatoryRecord(domain, record, 0)
}
...
func NewSPF(domain string, record string, count int) (SPF, error) {
	var spf SPF
	...
	return newSPFWithMandatoryRecord(domain, record, count)
}


// Create a new SPF record for the given domain using the provided string. If
// the provided string is not valid an error is returned.
func newSPFWithMandatoryRecord(domain string, record string, count int) (SPF, error) { // unixman, modified from NewSPF(), if record is empty will not lookup for it using getSPFRecord() but instead will return an error
	var spf SPF

	if record == "" {
		return spf, ErrEmptyRecord
	}
