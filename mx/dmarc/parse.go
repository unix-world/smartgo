...
// created by unixman, moved the parser methods here from lookup.go
// modified by unixman
// r.20260829
...
const (
	...
	txtRecordPrefixVal string = "DMARC1"

	TxtRecordPrefix    string = txtRecordPrefixVar + txtRecordPrefixVal
)

var (
	errUnsupportedVersion = errors.New("dmarc: unsupported DMARC version")
)
...
func Parse(txt string) (*Record, error) {
	...
	if params["v"] != txtRecordPrefixVal {
