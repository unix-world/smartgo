...
func HTMLCodeStripTags(htmlCode string) string {
	...
	//--
	return HTMLDecodeEntities(htmlCode)
	//--
} //END FUNCTION
...
func HTMLDecodeEntities(htmlCode string) string {
	...
	for k, v := range htmlEntities {
		htmlCode = StrReplaceAll(htmlCode, "&" + k + ";", string(v))
	} //end for
	//--
	matches, err := StrRegexFindAllMatches(REGEX_HTML_NUMERIC_ENTITY, htmlCode, 0)
	if(err != nil) {
		log.Println("[WARNING]", CurrentFunctionName(), "# Regex Failed:", err)
	} else if(len(matches) > 0) {
		for i:=0; i<len(matches); i++ {
			if(len(matches[i]) == 2) {
				//-- code below was taken and adapted from github.com/k3a/html2text/html2text.go # License MIT
				entity := matches[i][0] // do not trim
				digits := StrTrimWhitespaces(matches[i][1]) // just the digits of the entity
				if((digits != "") && (entity == "&#" + digits + ";")) { // extra control check
					var n int64
					var err error
					...
					if((err == nil) && ((n == 9) || (n == 10) || (n == 13) || (n > 31))) {
						htmlCode = StrReplaceWithLimit(htmlCode, entity, string(rune(n)), 1) // replace just the first occurence goes faster on larger texts
					} //end if
				} //end if
				//-- #end
			} //end if
...
var htmlEntities = map[string]rune{ // this map was taken from github.com/k3a/html2text/entity.go # License MIT
	"AElig":                           '\U000000C6',
	"AMP":                             '\U00000026',
	"Aacute":                          '\U000000C1',
	"Abreve":                          '\U00000102',
