...
// REQUIRE: webkitgtk4 >= 2.22 ; changed by unixman
...
func (v *WebView) RunJavaScript(script string, resultJsCallback func(err string, result string)) {
	var cCallback C.GAsyncReadyCallback
	var userData C.gpointer
	var err error
	if resultJsCallback != nil {
		callback := func(result *C.GAsyncResult) {
			...
			if jsResult == nil {
				...
				resultJsCallback("ERROR", msg)
				return
			}
			jsRawResult := C.GoString((*C.char)(C.jsc_value_to_string(C.webkit_javascript_result_get_js_value(jsResult))))
			resultJsCallback("OK", jsRawResult)
		}
