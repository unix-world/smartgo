...
func JwtNew(jwtSignMethod string, expirationMinutes int64, dom string, port string, userName string, userPrivKey string) (*JwtData, error) {
	...
	if(dom == "") {
		return nil, smart.NewError("Server Domain is Empty")
	} //end if
	port = smart.StrTrimWhitespaces(port)
	if(port == "") {
		return nil, smart.NewError("Server Port is Empty")
	} //end if
	//--
	userName = smart.StrTrimWhitespaces(userName)
	if(userName == "") {
		return nil, smart.NewError("User Name is Empty")
	} //end if
	//--
	userPrivKey = smart.StrTrimWhitespaces(userPrivKey)
	if(userPrivKey == "") {
		return nil, smart.NewError("User Private Key is Empty")
	} //end if
	//--
	dKeyLen, safeKey, errSafeKey := jwtSafeKey(jwtSignMethod, dom, port, userName, userPrivKey)
	if(errSafeKey != nil) {
		return nil, errSafeKey
	} //end if
	if(smart.StrTrimWhitespaces(safeKey) == "") {
		return nil, smart.NewError("SafeKey is Empty")
	} //end if
	if((dKeyLen <= 0) || (len(safeKey) != int(dKeyLen))) {
		return nil, smart.NewError("SafeKey have an Invalid Length")
	} //end if
	//--
	...
	if(token == nil) {
		return nil, smart.NewError("Token is NULL")
	} //end if
	switch(jwtSignMethod) {
		...
		case "H3S512": // sha3-512
			...
			if(len(safeKey) != 64) { // {{{SYNC-JWT-HS-KEY-LEN}}}
				return nil, smart.NewError("Derived Private Key (" + jwtSignMethod + ") ERR: `Key Size must be 64 bytes`")
			} //end if
			//-- #
		...
		case "Ed25519":
			...
			if(errK != nil) {
				return nil, smart.NewError("Private Key (" + jwtSignMethod + ") ERR: `" + errK.Error() + "`")
			} //end if
			if(pK == nil) {
				return nil, smart.NewError("Private Key is NULL")
			} //end if
			if(len(pbKey) != int(dKeyLen)) {
				return nil, smart.NewError("Public Key has an Invalid Size [" + smart.ConvertIntToStr(len(pbKey) * 4) + " bit]")
			} //end if
			publicKey = smart.StrTrimWhitespaces(smart.Base64Encode(string(pbKey)))
			if(publicKey == "") {
				return nil, smart.NewError("Public Key (" + jwtSignMethod + ") is Empty")
			} //end if
			//-- #
		...
		case "Ed448":
			...
			break
		default:
			return nil, smart.NewError("Unsupported JWT Algorithm: `" + jwtSignMethod + "`")
	} //end switch
	//--
	if(tokenErr != nil) {
		return nil, smart.NewError("Token Creation ERR: `" + tokenErr.Error() + "`")
	} //end if
	tokenString = smart.StrTrimWhitespaces(tokenString)
	if(tokenString == "") {
		return nil, smart.NewError("Token Creation Failed, Empty Data")
	} //end if
	//--
	...
	if(errVfyWithoutPubKey != nil) {
		return nil, errVfyWithoutPubKey
	} //end if
	//--
	data := &JwtData{
		Token: tokenString,
	...
	switch(jwtSignMethod) {
		...
		case "Ed25519":
			if(publicKey == "") {
				...
				if(len(pbKey) != int(dKeyLen)) {
					return smart.NewError("Public Key has an Invalid Size [" + smart.ConvertIntToStr(len(pbKey) * 4) + " bit]")
				} //end if
				publicKey = smart.StrTrimWhitespaces(smart.Base64Encode(string(pbKey)))
				if(publicKey == "") {
...
func jwtSafeKey(jwtSignMethod string, dom string, port string, userName string, userPrivKey string) (uint16, string, error) {
	...
	if((len(safeKey) != int(dKeyLen)) || (len(safeKey) != len(smart.StrTrimWhitespaces(safeKey)))) {
		return dKeyLen, "", smart.NewError("Derived Key Length is Invalid: [" + smart.ConvertIntToStr(len(safeKey) * 4) + " bit]")
	} //end if
	//--
