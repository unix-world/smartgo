...
func gzipHandleWrite(w http.ResponseWriter, r *http.Request, fn http.HandlerFunc) {
	//--
	defer smart.PanicHandler()
	//--
	// IMPORTANT:
	...
	w.Header().Set("z-compressed", "gzip")
...
func gzipHandler(fn http.HandlerFunc) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		//--
		defer smart.PanicHandler()
		//-- check if the client can accept the gzip encoding, otherwise fall back to plain uncompressed
		...
		if(smart.InListArr(CONTENT_TYPE_GZIP, arrList)) { // client accepts gzip
			gzipHandleWrite(w, r, fn) // gzip
			return
		} //end if
		//-- plain, no support for gzip
		fn(w, r) // fallback to plain uncompressed, client does not accept gzip
		return
		//--
	} //end fx
	//--
