/**
 * QUnit 2.20.0.1 (modified by unixman r.20260831)
 * https://qunitjs.com/
 *
...
//	* jQuery 3.6.0 ready (fixed XHTML Tags)
...
(function () {
	...
	function extractStacktrace(e, offset) {
		//-- unixman
		if(config.uxmUniqueShowDebugStack !== true) {
			return null;
		} //end if
		//-- #
	...
	(function () {
		...
		function appendToolbar(beginDetails) {
			...
			if (toolbar) {
				...
				//-- fix by unixman (hide filter search modules)
		...
		function appendTest(name, testId, moduleName) {
			...
			if (testId !== undefined) {
				...
				//-- fix by unixman (allow test re-run)
			//	testBlock.appendChild(rerunTrigger);
				if(config.uxmAllowRerun !== false) {
					testBlock.appendChild(rerunTrigger);
				} // end if
				//-- #end fix
			}
