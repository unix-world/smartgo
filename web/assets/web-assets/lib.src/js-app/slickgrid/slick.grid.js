 ...
 * SlickGrid v1.4.3.2.smart.20260902.1858 : Smart.Framework
 ...
 * 			19. Add column option isNameHtml, default to FALSE ; column Name is rendered as TEXT by default, needs explicit set to isNameHtml=TRUE to be rendered as HTML Code
 * 			20. Code optimizations, support for jQuery 3.7.1+
 * 			21. Fix sorting indicator as display block, position absolute to work with html blocks inside header, ex: [center] ... [/center]
 * 			22. Add option to disable tooltip on cells
 *
 *
 * TODO:
 * - frozen columns
 ...
 *     isNameHtml          - (default FALSE) ; if TRUE the column name is rendered as HTML otherwise as TEXT (unixman)
...
(function($) { // ES6
	...
	const SlickGrid = function(container,data,columns,options) { // START CLASS
		...
		const columnDefaults = {
			name: '',
			isNameHtml: false, // column Name is rendered as TEXT by default, enabling HTML must be set explicit
			resizable: true,
			sortable: false,
		...
		function init() {
			...
			//--
			$viewport = $('<div class="slick-viewport" tabIndex="0" style="width:100%;overflow-x:auto;overflow-y:auto;outline:0;position:relative;" hideFocus></div>').appendTo($container);
		...
		function createColumnHeaders() {
			...
			for(i = 0; i < columns.length; i++) {
				...
				let header = $('<div class="slick-header-column c' + i + ' slick-state-default' + (m.cssHeadClass ? ' ' + escapeHtml(m.cssHeadClass) : '') + '" id="' + escapeHtml('' + uid + m.id) + '"></div>')
					.html('<span class="slick-column-name">' + (m.isNameHtml ? m.name : escapeHtml(m.name)) + '</span>') // header can be HTML
					.width((m.currentWidth || m.width) - headerColumnWidthDiff)
					.attr('title', (!!options.disableToolTip ? '' : (m.toolTip || m.name || '')))
					.data('fieldId', m.id)
					.appendTo($headers);
				...
				//	header.append('<span class="slick-sort-indicator"></span>');
					header.append('<span class="slick-sort-indicator" style="display:block; position:absolute; right:5px; top:10px;"></span>');
				}
